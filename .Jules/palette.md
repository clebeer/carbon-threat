...
## 2026-06-09 - Setup Wizard Disabled Buttons
**Learning:** The application's Setup Wizard step 3 had 'Review Setup' buttons that were visually styled as disabled when required fields were empty, but they lacked the semantic  HTML attribute and a tooltip () explaining why they were inactive.
**Action:** Adding the  attribute makes the inactive state semantically correct for screen readers and keyboard navigation, while the  attribute provides necessary context for mouse users on what conditions must be met to proceed.
