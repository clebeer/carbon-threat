"""
Test Cases Generation — adapted from STRIDE-GPT for headless API usage.
...
def create_test_cases_prompt(threat_model):
    prompt = """Act as a cyber security expert with more than 20 years experience of creating security test cases for a wide range of applications.

You are provided with a threat model and must create specific, actionable test cases for each threat identified.
...
Provide your test cases as a JSON object with the key "test_cases" containing an array of objects, each with:
- "Threat Type": the threat type
- "Scenario": the scenario
- "Test Cases": an array of test case objects, each with "name" and "description"
      ...
      "Test Cases": [
        {
          ...
          "description": "Attempt to authenticate with invalid credentials and verify the system rejects the attempt"
        }
      ]
    }
  ]
