...
export async function syncAttackData(logId) {
  await db('attack_sync_log').where({ id: logId }).update({ status: 'running' });
  ...
  try {
    ...
    async function upsertObjects(rows) {
      ...
      await db('attack_objects')
        ...
        .merge(['name', 'description', 'platforms', 'kill_chain_phases',
    ...
    const techniqueRows = techniques
      ...
      .map(o => ({
        ...
        url:              getAttackUrl(o),
        stix_id:          o.id,
        is_deprecated:    !!o.x_mitre_deprecated,
        is_revoked:       !!o.revoked,
    ...
    const subTechs = await db('attack_objects').where({ type: 'sub-technique' }).select('id', 'attack_id');
    for (const sub of subTechs) {
      ...
      const parent = await db('attack_objects')
        ...
        .first();
      if (parent) {
        await db('attack_objects').where({ id: sub.id }).update({ parent_id: parent.id });
      }
    }
    ...
    // ── Finalise sync log ──────────────────────────────────────────────────
    await db('attack_sync_log').where({ id: logId }).update({
      status:                'complete',
...
export async function getSyncStatus() {
  const last = await db('attack_sync_log')
    ...
    .first();
  const count = await db('attack_objects').count('id as n').first();
  return {
    lastSync:   last ?? null,
    totalObjects: parseInt(count?.n ?? '0', 10),
    isSynced:   !!last && last.status === 'complete',
  };
}
...
export async function getTechniques(filters = {}) {
  ...
  const total = await q.clone().count('ao.id as n').first();
  const rows  = await q
    .select('ao.*')
    .orderByRaw(`
      CASE ao.type
...
export async function createMapping({ threatId, techniqueId, modelId, confidence, notes, userId }) {
  ...
  if (!/^[0-9a-f]{8}-/.test(techniqueId)) {
    const tech = await db('attack_objects').where({ attack_id: techniqueId }).first();
...
export async function deleteMapping(id) {
  const deleted = await db('attack_threat_mappings').where({ id }).delete();
  return deleted > 0;
}
...
export async function generateReport(modelId, format = 'json') {
  ...
  const lines = [
    ...
    ...coverage.tactics.map(t =>
      `| ${t.name} | ${t.attack_id} | ${cov(t.covered)} | ${t.relatedStrideCategories.join(', ')} | ${t.mappingCount} |`
    ),
    ``,
    `## STRIDE Threats`,
    ``,
    ...
    `|-------|--------|----------|--------|`,
    ...coverage.threats.map(t =>
      `| ${t.title} | ${t.stride_category} | ${sev(t.severity)} ${t.severity} | ${t.status} |`
    ),
    ``,
    `## ATT&CK Mappings`,
    ``,
  ...
  if (coverage.mappings.length === 0) {
    ...
    lines.push(
      ...
      `|--------|-----------|-----------|------------|`,
      ...coverage.mappings.map(m =>
        `| ${m.threat_title ?? '(model-level)'} | [${m.technique_name}](${m.technique_url ?? ''}) | ${m.technique_attack_id} | ${m.confidence} |`
      )
    );
  }
  ...
  if (coverage.recommendations.length === 0) {
    ...
    lines.push(
      ...
      `|--------|-----------------|----------------|`,
      ...coverage.recommendations.map(r =>
        `| ${r.threat_title} | ${r.stride_category} | ${r.tactic_name} (${r.tactic_id}) |`
      )
    );
  }
