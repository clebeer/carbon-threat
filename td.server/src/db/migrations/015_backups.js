...
export const up = async (knex) => {
  await knex.schema.createTable('backups', (t) => {
    ...
    t.enu('status', ['pending', 'running', 'complete', 'error']).notNullable().defaultTo('pending');
    t.enu('storage_type', ['local', 'sftp', 'gdrive']).notNullable().defaultTo('local');
  ...
  await knex.schema.createTable('backup_schedules', (t) => {
    ...
    t.uuid('org_id').references('id').inTable('organizations').onDelete('CASCADE');
    t.string('name', 255).notNullable();
    t.enu('frequency', ['daily', 'weekly', 'monthly']).notNullable().defaultTo('daily');
