...
unique();
    ...
    t.text('description');
    t.jsonb('icon_manifest').notNullable().
defaultTo('{}');
    t.jsonb('threat_matrix').notNullable().
defaultTo('{}');
    t.boolean('is_builtin').notNullable().
defaultTo(true);
  ...
  await knex.schema.createTable('domain_templates', (t) => {
    t.uuid('id').primary().
defaultTo(knex.raw('gen_random_uuid()'));
    t.uuid('pack_id').notNullable().
references('id').
inTable('domain_packs').
onDelete('CASCADE');
    t.string('name', 255).notNullable();
    t.text('description');
    t.jsonb('diagram_json').notNullable().
defaultTo('{}');
    t.timestamps(true, true);
  });
};
