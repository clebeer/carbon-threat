...
export async function up(knex) {
  ...
  await knex('integration_configs').where('platform', 'jules').
delete();
...
defaultTo(knex.raw('gen_random_uuid()'));
    ...
    t.string('finding_type', 20).notNullable().
defaultTo('osv');
    t.string('source_name', 500).notNullable();
    t.text('prompt').notNullable();
    t.string('automation_mode', 30).notNullable().
defaultTo('AUTO_CREATE_PR');
