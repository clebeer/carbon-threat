...
function applyFilters(query, { action, entity_type, user_id, from, to }) {
  ...
  if (action) {filteredQuery = filteredQuery.where('action', action);}
  if (entity_type) {filteredQuery = filteredQuery.where('entity_type', entity_type);}
  if (user_id) {filteredQuery = filteredQuery.where('user_id', user_id);}
  if (from) {filteredQuery = filteredQuery.where('created_at', '>=', new Date(from));}
  if (to) {filteredQuery = filteredQuery.where('created_at', '<=', new Date(to));}
